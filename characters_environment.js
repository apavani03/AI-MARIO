...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press the play button and start controlling with your nose!!! ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatus(character){
  ...
  // 1st condition  Game Status  = Start  
  // 2nd condition PoseNEt - Nose X or NoseY shpuld be there i.e. noseX != blank value
...
//AND - &&
// Not  Equal- !=
//Equal - == check value if it is equal
// === to check if the vale along with the datatype is same
...
function getCoins(coin,character){
  if( character.overlap(coin) && character.live && coin.get==false){
    character.coins+=1; // character.coins = character.coins + 1 ;
    coin.get=true;
    mario_coin.play();
  };
}
...
//!keyDown(control.left)&&!keyDown(control.right)&&!keyDown(control.up)
// nose_X >350 && nose_x <450 && nose_y >200
    if(nose_X >350 && nose_X <450 && nose_Y >200){ 
